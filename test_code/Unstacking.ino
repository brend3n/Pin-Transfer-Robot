...
void measure_microplate_height() {
  int val;
  while(true){
  ...
  Serial.print("z2_start:"); Serial.println(z2_start);
}
...
// plate_number represents the number of the topmost plate
// on the stack from the bottom.
void pop_stack(int num_plates, int offset, int plate_number) {
  gripper(OPEN, servo);
  delay(2000);
  
  motor_z2.setSpeed(SPEED_Z2);
  motor_z2.runToNewPosition(-2090 + offset * plate_number);
  gripper(CLOSE, servo);
  delay(1000);

  // Just a safe ballpark
  motor_z2.move(2 * offset);
  motor_z2.setSpeed(SPEED_Z2);
  motor_z2.runToPosition();
  delay(1000);
...
void setup(){
  ...
  unstack(5, MICROPLATE_OFFSET, OPTIMAL_BASELINE_Z);
  ...
  // for (int num_plates = 2; num_plates >= 0; num_plates--) {
  //   pop_stack(num_plates, MICROPLATE_OFFSET, num_plates - 1);
  // }

  // test_baseline_height(-1771);
  // measure_microplate_height();
